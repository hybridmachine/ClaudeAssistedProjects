export class PatternLoader {
    ...
    parseRLE(rleContent) {
        ...
        const rleDataLines = lines.filter(line => !line.startsWith('#') &&
            !line.startsWith('x ') &&
            line.length > 0);
    ...
    resizePattern(pattern, newWidth, newHeight) {
        const resized = Array(newHeight).fill(null).map(() => Array(newWidth).fill(false));
    ...
    rotatePattern(pattern, clockwise = true) {
        const height = pattern.length;
        const width = pattern[0].length;
    ...
    flipPattern(pattern, horizontal = true) {
        const height = pattern.length;
        const width = pattern[0].length;
        const flipped = Array(height).fill(null).map(() => Array(width).fill(false));
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                if (horizontal) {
